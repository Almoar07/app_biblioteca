...
class LiveAuthorTable extends Component
{
    use WithPagination;
    ...
    public $countries = [];
    protected $listeners = [
        'refreshAuthorTable' => '$refresh',
        'successEditingAuthor' => 'showSuccessAlert',
    ...
    public function render()
    {
        ...
        $authors = Autor::where('nombre', 'LIKE', "%{$this->search}%")->orWhere('apellido_paterno', 'LIKE', "%{$this->search}%")->orWhere('apellido_materno', 'LIKE', "%{$this->search}%")->orWhere('fecha_nacimiento', 'LIKE', "%{$this->search}%")->orWhere('created_at', 'LIKE', "%{$this->search}%")->orWhere('nacionalidad', 'LIKE', "%{$this->search}%");;

        /* Se verifica si el campo de ordenamiento y la dirección no son nulos y se ordena los datos */
        if ($this->sortField && $this->sortDirection) {
            $authors = $authors->orderBy($this->sortField, $this->sortDirection);
        } else {
        ...
        return view('livewire.live-author-table', [
            'authors' => $authors,
    ...
    public function openAuthorModal($mode, $id)
    {
        $this->dispatch('openAuthorModal', $mode, $id); // Dispara el evento para abrir el modal de creación de usuario
    }

    public function showDeleteConfirmation($rut, $name, $lastname, $lastname2)
    {
        $this->dispatch('delete-attempt', model: "Author", id: $rut, title: "Eliminando autor", text: "¿Está seguro de eliminar al autor:\n{$name} {$lastname} {$lastname2}?\nEsta acción solo la puede deshacer un administrador.");
    }
