...
class LiveCategoryTable extends Component
{
    use WithPagination;
    ...
    protected $listeners = [
        ...
        'successEditingCategory' => 'showSuccessAlert',
    ...
    public function render()
    {
        ...
        $categories = Categoria::where('nombre_categoria', 'LIKE', "%{$this->search}%")
            ->orWhere('descripcion_categoria', 'LIKE', "%{$this->search}%");


        /* Se verifica si el campo de ordenamiento y la dirección no son nulos y se ordena los datos */
        if ($this->sortField && $this->sortDirection) {
            $categories = $categories->orderBy($this->sortField, $this->sortDirection);
        } else {
        ...
        return view('livewire.live-category-table', [
    ...
    public function openCategoryModal($mode, $id)
    {
        $this->dispatch('openCategoryModal', $mode, $id); // Dispara el evento para abrir el modal de creación de categoría
    }

    public function showDeleteConfirmation($rut, $name)
    {
        $this->dispatch('delete-attempt', model: "Category", id: $rut, title: "Eliminando categoria", text: "¿Está seguro de eliminar al autor:\n{$name}?\nEsta acción solo la puede deshacer un administrador.");
    }
