        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div x-show="open" x-transition:enter="ease-out duration-300">
                            ...
                            <div class="border-b border-gray-200 dark:border-gray-700 pb-3 flex items-start">
                                ...
                                <div>
                                    <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">
                                        {{ $mode === 'create' ? 'Crear editorial' : 'Editar editorial' }}</h3>
                                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                        {{ $mode === 'create' ? 'Ingrese los datos de la nueva editorial' : 'Complete o modifique información de la editorial' }}
                                    </p>
                                </div>
                            </div>
                                    ...
                                    <div class="space-y-5">
                                        <fieldset class="space-y-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300 px-2">
                                                Información de la editorial</p>

                                            <!-- Nombre -->
                                            <div>
                                                <x-input-label for="publisherName" :value="__('Nombre')" class="sr-only" />
                                                <x-text-input wire:model="publisherName" id="publisherName"
                                                    name="publisherName" type="text" class="block w-full"
                                                    placeholder="Nombre" required autofocus autocomplete="given-name" />
                                                <x-input-error class="mt-1 text-sm" :messages="$errors->get('publisherName')" />
                                            </div>
                ...
                <div class=" bg-gray-50 dark:bg-gray-900 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                    ...
                    <button type="button" wire:click={{ $mode === 'create' ? 'createPublisher' : 'updatePublisher' }}
