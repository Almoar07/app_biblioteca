<div>
    ...
    <div class="mx-auto px-16 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Gestión de Autores</h1>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                ...
                <div class="relative w-full md:w-64">
                    ...
                    <input type="text" wire:model.live="search" placeholder="Buscar autores..."
                ...
                <!-- Botón Añadir Autor -->
                <button wire:click="openAuthorModal('create', null)"
                    class="bg-blue-600
                ...
                <table class="min-w-full table-fixed divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="whitespace-nowrap">
                            <th
                                ...
                                <div class="flex items-center justify-between">
                                    <span>Nombre</span>
                                    <button wire:click="sortBy('nombre')" class="ml-2">
                                        <i
                                            class="fa-solid fa{{ $sortField === 'nombre' ? $icon : '-circle' }} text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"></i>
                                    </button>
                                </div>
                            </th>
                            ...
                            <th
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <button wire:click="sortBy('nacionalidad')" class="ml-2">
                                        <i
                                            class="fa-solid fa{{ $sortField === 'nacionalidad' ? $icon : '-circle' }} text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"></i>
                                    </button>
                                </div>
                            </th>
                            <th
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @foreach ($authors as $author)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700" wire:key="author-{{ $author->id }}">
                                <td
                                    class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                    {{ $author->nombre }} {{ $author->apellido_paterno }}
                                    {{ $author->apellido_materno }}</td>

                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ \Carbon\Carbon::parse($author->fecha_nacimiento)->translatedFormat('j \d\e F \d\e Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    @php
                                        $flag = '';
                                    ...
                                    @if ($flag)
                                        <img src="{{ $flag }}" alt="Flag"
                                            class="inline h-5 w-auto mr-2 align-middle border-gray-300 dark:border-gray-600" />
                                    @endif

                                    {{ ucfirst($author->nacionalidad) }}
                                </td>

                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ $author->created_at->translatedFormat('j \d\e F \d\e Y \a \l\a\s H:i') }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <div class="flex gap-2">
                                        ...
                                        <a wire:click="openAuthorModal('edit',{{ $author->id_autor }})"
                                        ...
                                        @if (auth()->user()->isAdmin())
                                            ...
                                            <button class="text-red-600 hover:text-red-900 dark:text-red-400"
                                                title="Eliminar"
                                                wire:click="showDeleteConfirmation({{ $author->id_autor }}, '{{ $author->nombre }}', '{{ $author->apellido_paterno }}', '{{ $author->apellido_materno }}')">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                                    viewBox="0 0 24 24" stroke="currentColor">
