        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div x-show="open" x-transition:enter="ease-out duration-300">
                                ...
                                <div class="mt-5 grid grid-cols-1 sm:grid-cols-1 gap-5">
                                    ...
                                    <div class="space-y-5">
                                        <fieldset class="space-y-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                                            ...
                                            <!-- Nombre -->
                                            <div>
                                                <x-input-label for="authorName" :value="__('Nombre')" class="sr-only" />
                                                <x-text-input wire:model="authorName" id="authorName" name="authorName"
                                                    type="text" class="block w-full" placeholder="Nombre" required
                                                    autofocus autocomplete="given-name" />
                                                <x-input-error class="mt-1 text-sm" :messages="$errors->get('authorName')" />
                                            </div>

                                            <!-- Apellidos -->
                                            <div class="grid grid-cols-2 gap-3">
                                                <div>
                                                    <x-input-label for="authorLastname" :value="__('Paterno')"
                                                        class="sr-only" />
                                                    <x-text-input wire:model="authorLastname" id="authorLastname"
                                                        placeholder="Apellido paterno" class="block w-full" required />
                                                </div>
                                                <div>
                                                    <x-input-label for="authorLastname2" :value="__('Materno')"
                                                        class="sr-only" />
                                                    <x-text-input wire:model="authorLastname2"
                                            ...
                                            <!-- Fecha de Nacimiento -->
                                            <div>
                                                <x-input-label for="authorBirthday" :value="__('Fecha nacimiento')" class="sr-only" />
                                                <x-text-input wire:model="authorBirthday" id="authorBirthday"
                                            ...
                                            <div>
                                                ...
                                                }" x-init="updateFlag(@entangle('authorNationality'))"
                                                    class="flex items-center space-x-3">
                                                    {{-- Bandera fija con espacio reservado --}}
                                                    <div
                                                        class="w-14 h-10 flex-shrink-0 shadow overflow-hidden bg-gray-200 dark:bg-gray-600">
                                                        <img x-show="selectedFlag" :src="selectedFlag"
                                                            alt="Bandera del país" class="w-full h-full object-cover" />
                                                    </div>

                                                    {{-- Select de países --}}
                                                    <select wire:model="authorNationality" id="authorNationality"
                                                        x-on:change="updateFlag($event.target.value)"
                                        ...
                                        {{-- <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                                            <x-input-label for="authorNationality" :value="__('Nacionalidad')" />
                                            <select wire:model="authorNationality" id="authorNationality"
                                                class="form-select">
                ...
                <div class=" bg-gray-50 dark:bg-gray-900 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                    ...
                    <button type="button" wire:click={{ $mode === 'create' ? 'createAuthor' : 'updateAuthor' }}
