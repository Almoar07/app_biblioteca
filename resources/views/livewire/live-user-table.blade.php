    ...
    <div class="mx-auto px-16 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                ...
                <button wire:click="openUserModal('create', null)"
                    class="bg-blue-600
                ...
                @if (auth()->user()->isAdmin())
                    {{-- Botón ver registros borrados --}}
                    {{-- <button wire:click="openDeletedUserModal"
                        class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2.5 rounded-lg inline-flex items-center gap-2 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 whitespace-nowrap">
                        <svg class="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 18 18">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 6h12M6 6v10a1 1 0 001 1h4a1 1 0 001-1V6m-8 0V4a2 2 0 012-2h4a2 2 0 012 2v2m-8 0h8" />
                        </svg>
                        Ver eliminados
                    </button> --}}
                @endif
                        ...
                        <tr class="whitespace-nowrap">
                            <th
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <button wire:click="sortBy('rut_usuario')" class="ml-2">
                                        <i
                                            class="fa-solid fa{{ $sortField === 'rut_usuario' ? $icon : '-circle' }} text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"></i>
                                    </button>
                                </div>
                            </th>
                            <th
                                ...
                                <div class="flex items-center justify-between">
                                    <span>Nombre</span>
                                    <button wire:click="sortBy('name')" class="ml-2">
                                        <i
                                            class="fa-solid fa{{ $sortField === 'name' ? $icon : '-circle' }} text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"></i>
                                    </button>
                                </div>
                            </th>
                            <th
                            ...
                            <th
                                ...
                                <div class="flex items-center justify-between">
                                    <span>Teléfono</span>
                                    <button wire:click="sortBy('phone')" class="ml-2">
                                        <i
                                            class="fa-solid fa{{ $sortField === 'phone' ? $icon : '-circle' }} text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"></i>
                                    </button>
                                </div>
                            </th>
                            <th
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <button wire:click="sortBy('birthday')" class="ml-2">
                                        <i
                                            class="fa-solid fa{{ $sortField === 'birthday' ? $icon : '-circle' }} text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"></i>
                                    </button>
                                </div>
                            </th>
                            <th
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <button wire:click="sortBy('tipo_usuario')" class="ml-2">
                                        <i
                                            class="fa-solid fa{{ $sortField === 'tipo_usuario' ? $icon : '-circle' }} text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"></i>
                                    </button>
                                </div>
                            </th>
                            <th
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @foreach ($users as $user)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700"
                                wire:key="user-{{ $user->rut_usuario }}">
                                <td
                                    class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                    {{ $user->rut_usuario }}</td>
                                <td
                                    class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                    {{ $user->name }} {{ $user->lastname }} {{ $user->lastname2 }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ $user->email }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ $user->phone }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ \Carbon\Carbon::parse($user->birthday)->translatedFormat('j \d\e F \d\e Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ $user->tipo_usuario }}
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ $user->created_at->translatedFormat('j \d\e F \d\e Y \a \l\a\s H:i') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ $user->status }}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <div class="flex gap-2">
                                        ...
                                        <a wire:click="openUserModal('edit',{{ $user->id }})"
                                        ...
                                        @if (auth()->user()->isAdmin())
                                            ...
                                            <button class="text-red-600 hover:text-red-900 dark:text-red-400"
                                                title="Eliminar"
                                                wire:click="showDeleteConfirmation({{ $user->id }}, '{{ $user->name }}', '{{ $user->lastname }}', '{{ $user->lastname2 }}')">
